...
bowtie = {
    doc "align with bowtie (best mode), selecting only uniquely mapping reads"
    output.dir = PROJECT + "/mapped"

    def BOWTIE_FLAGS = "-q --sam --phred33-quals --best --strata --tryhard --chunkmbs 256 --no-unal" +
        " -v " + max_mismatches +
        " -m " + max_multimap +
        " -p " + threads
    ...
    transform(".fastq.gz") to (".bam") {
        exec """
            ml bowtie/1.2.2 samtools/1.9 &&
            export TMP=/jobdir/\${SLURM_JOBID}/ &&
            zcat $input | bowtie $BOWTIE_FLAGS $ref - | samtools view -bhSu - | samtools sort -O bam -@ $threads -T \${TMP}/\$(basename $output.prefix)_bowtie1_sort - > $output
        """, "bowtie"
    }
}
...
Bpipe.run {
    "%.fastq.gz" * [
        ...
        bowtie.using(max_mismatches: 2, max_multimap: 1, threads: THREADS, ref: HG19_REF) +
        bam2bw
    ]
}
